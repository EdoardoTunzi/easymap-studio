  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between">
        <span className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Layers</span>
      ...
      <ul className="flex max-h-fit flex-col gap-1 overflow-y-auto">
        {display.map((layer, di) => {
          ...
          return (
            ...
            >
              ...
              <span className={cn("flex-1 truncate text-sm", layer.visible ? "text-foreground" : "text-muted-foreground line-through")}>{layer.name}</span>
              ...
              {paletteLoopLayerIds.includes(layer.id) && <Repeat className="size-3 shrink-0 animate-pulse text-primary" aria-label="Loop palette attivo" />}
              <span className="shrink-0 text-[10px] uppercase text-muted-foreground">{layer.shaderName ? layer.blendMode : ""}</span>
              <button
                type="button"
                onClick={(e) => {
                  e.stopPropagation();
