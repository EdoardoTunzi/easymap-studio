// NAME: Radial Kaleido 5
uniform float seed; // @min 0.0 @max 100.0 @default 0.0
uniform float segments; // @min 2.0 @max 16.0 @default 6.0
uniform float segments2; // @min 2.0 @max 24.0 @default 11.0
uniform float layerMix; // @min 0.0 @max 1.0 @default 0.5
uniform float counterSpin; // @min -2.0 @max 2.0 @default 0.7
uniform float rings; // @min 1.0 @max 20.0 @default 8.0
uniform float swirlAmount; // @min 0.0 @max 2.0 @default 0.6
uniform float colorShift; // @min 0.0 @max 10.0 @default 3.0
uniform float hue; // @min 0.0 @max 1.0 @default 0.4
uniform float sat; // @min 0.0 @max 2.0 @default 1.0
uniform float intensity; // @min 0.0 @max 1.0 @default 0.85
uniform float speed; // @min 0.0 @max 3.0 @default 1.0

vec3 palette(float t) {
  vec3 base = vec3(0.5) + vec3(0.5) * cos(6.28318 * (vec3(0.4, 0.2, 0.6) * t + vec3(0.1, 0.4, 0.7) + hue));
  return mix(vec3(dot(base, vec3(0.299, 0.587, 0.114))), base, sat);
}

float fold(vec2 p, float segs, float spin, float stime, float mod0) {
  float r = length(p);
  float a = atan(p.y, p.x) + spin * stime;
  float seg = 6.28318 / segs;
  a = abs(mod(a, seg) - seg * 0.5);
  a += swirlAmount * r * sin(stime * 0.5 + r * 3.0 + mod0);
  return 0.5 + 0.5 * sin(a * 6.0 + r * rings - stime * 1.5);
}

vec4 processColor(sampler2D tex, vec2 uv, float time, vec2 resolution) {
  float stime = time * speed + seed * 9.2;
  vec4 source = texture2D(tex, uv);
  float lum = dot(source.rgb, vec3(0.299, 0.587, 0.114));
  vec2 p = uv * 2.0 - 1.0;
  p.x *= resolution.x / resolution.y;
  float r = length(p);
  // due rosoni sovrapposti che girano in verso opposto: l'incrocio crea il battimento
  float base = fold(p, segments, 0.0, stime, source.r * colorShift);
  float over = fold(p, segments2, counterSpin * 0.3, stime, source.b * colorShift);
  float pattern = mix(base, base * over * 2.0, layerMix);
  vec3 col = palette(pattern + r + source.g * colorShift * 0.2) * pow(clamp(pattern, 0.0, 1.5), 2.0) * 2.0;
  vec3 blended = mix(source.rgb, col, intensity * smoothstep(0.0, 0.4, lum));
  return vec4(blended, source.a);
}
