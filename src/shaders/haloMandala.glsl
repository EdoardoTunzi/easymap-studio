...
vec3 palette(float t) {
  return vec3(0.5) + vec3(0.5) * cos(6.28318 * (vec3(0.4, 0.3, 0.5) * t + vec3(0.6, 0.3, 0.2)));
}

vec4 processColor(sampler2D tex, vec2 uv, float time, vec2 resolution) {
  float stime = time * speed + seed * 14.2;
  ...
  for (int i = 0; i < 5; i++) {
    ...
    acc += abs(cos(a * petals * 0.5 * fi + stime + source.r * colorShift)) * exp(-r * fi);
  }
  vec3 col = palette(acc + r + source.g * colorShift * 0.2) * acc * 2.5;
