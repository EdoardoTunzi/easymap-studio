...
- [x] Shader **SD Shape Tunnel** (`src/shaders/sdShapeTunnel.glsl`): il profilo esterno della sagoma replicato in copie decrescenti che collassano verso il centro dell'oggetto (tunnel dalla forma dell'asset). Due modalità di ciclo (continuo / a blocco con nascite in sequenza), prospettiva regolabile, torsione progressiva, centro sul **baricentro** della sagoma con scostamento X/Y. Introdotto per questo l'uniform globale `uShapeCentroid`, calcolato su CPU alla decodifica dell'immagine (`computeCentroid` in `mediaTexture.ts`); per video/camera/GIF resta il centro del quad. Libreria a 108 effetti
- [ ] SD Shape Tunnel: valutare la reattività audio (nascita di una nuova copia sul beat con `easyvj_level`) — rimandata su richiesta dell'utente, prima si prova l'effetto così com'è
- [ ] `uShapeCentroid` per le sorgenti animate (video/camera/GIF): oggi resta (0.5, 0.5) perché il ricalcolo costerebbe una lettura di pixel per frame. Si potrebbe aggiornare a bassa frequenza (una volta al secondo) se emerge il bisogno
- [x] SD Shape Tunnel: controllo `holeFill` ("solo contorno esterno") — spegne i bordi interni (fori, finestre, dettagli della texture) misurando l'immersione del punto nella sagoma su un anello largo. È un raggio, non un interruttore: si alza finché i dettagli da ignorare spariscono, perché un foro più grande del raggio resta indistinguibile da un profilo esterno
- [x] SD Shape Tunnel: `ringDistance` (distanza fra una copia e la successiva, frazione del percorso; max 1 perché oltre la spaziatura diventerebbe irregolare) e nascita delle copie sul **bordo esatto** della sagoma — il fade-in di nascita era lungo 0.12 e le rendeva visibili solo a tunnel già iniziato, ora è 0.02
- [x] SD Shape Tunnel: `ringDistance` ricalibrato come **profondità del percorso** (`u = v * ringDistance`) invece che compressione degli offset di partenza. Nella prima versione, con distanza ridotta, la modalità "continuo" generava un plotone seguito da un vuoto rotante ed era indistinguibile da "a blocco": sembrava che il selettore delle modalità non funzionasse più
- [x] Skill `shadcn` installata (`.claude/skills/shadcn` → `.agents/skills/shadcn`) e audit delle sue regole sui 39 `.tsx` applicativi. Conformi già oggi: spacing con `gap-*` (zero `space-x/y-*`), `cn()` ovunque, nessun ternario in `className`, un solo override `dark:`, z-index mai su overlay shadcn
- [x] `MediaUploader.tsx` e `OutputLauncher.tsx`: icone dei `Button` da `className="size-*"` a `data-icon="inline-start"`, e "Ripristina" ripulito dagli override (`h-7 gap-1.5 px-2 text-xs`) che duplicavano a mano il variant `size="sm"`
- [x] Altre 25 icone → `data-icon` in 11 file (`TopToolbar`, `EffectsPanel`, `PalettePanel`, `MaskPanel`, `FxControlsPanel`, `PlaylistBar`, `LayerList`, `LayerProperties`, `AudioInputPanel`, `ProjectsPanel`, `CameraPicker`), più la rimozione di `shrink-0` già garantito dalla base del Button
- [x] Chiarito l'ambito reale della regola: `data-icon` serve solo dove il Button ha **icona + testo** (è il padding asimmetrico fra i due). Le 69 occorrenze contate nell'audit erano gonfiate: 35 sono `size="icon*"` dove non si applica. `MappingControls.tsx` è interamente icon-only e non va toccato, malgrado fosse in cima alla lista
- [ ] 5 Button icona+testo dove la classe scritta a mano **non** coincide col variant, quindi toglierla cambia la dimensione a video: `CameraPicker.tsx:198,202,211` (`size-3.5` su size default, che darebbe `size-4`), `MaskPanel.tsx:153` (`size-4` su `sm`, che darebbe `size-3.5`), `FxControlsPanel.tsx:54` (`size-3` su `sm`). Decidere caso per caso se era una scelta o una svista
- [ ] Icone dentro Button `size="icon-xs"` con `size-3.5` mentre il variant darebbe `size-3` (14px contro 12px): sono ~20, quasi tutte in `MappingControls.tsx`. Uniformarle è una decisione di design sulla toolbar di mapping, non una correzione — la regola della skill ammette le dimensioni custom volute
- [ ] Altri 11 `Button` con override di altezza/tipografia (`h-7 … text-xs`, `h-6 … text-[11px]`) che riscrivono i variant `sm` / `xs`
- [x] 2 `SelectItem` fuori da `SelectGroup` (`CameraPicker.tsx`, `LayerProperties.tsx`)
- [x] `h-N w-N` uguali → `size-N`: 4 maniglie in `CornerPinOverlay.tsx` + 1 pulsante in `EffectsPanel.tsx`
- [x] 14 classi che riscrivevano il variant (`h-8` su default, `h-7` su `sm`, `text-sm` su default che è già la base) rimosse dai `Button`
- [x] `text-xs` rimosso da 13 `Button` con `size="sm"`: ora la tipografia la decide il variant (`text-[0.8rem]`, 12,8px). In `TopToolbar.tsx` stava dentro `cn()`, rimosso a parte
- [x] **Deciso dall'utente: i 44 colori raw restano** — la palette dell'app è costruita su questi valori e sul canvas sono marker che devono restare leggibili sopra la proiezione. Non riproporre come difetto
- [x] **Deciso dall'utente: i 5 divisori `border-t` restano** — `Separator` richiederebbe `data-horizontal:w-auto` più l'override del colore, cioè più codice per lo stesso risultato. Non riproporre come difetto
- [ ] `FxControlsPanel.tsx:54`: `h-6` + `text-[11px]` su `size="sm"`. Il variant giusto sarebbe `size="xs"` (h-6, gap-1, px-2, text-xs), ma porta il testo da 11px a 12px e il padding da 1.5 a 2
- [ ] 17 `<button>` nativi contro 88 `<Button>`: verificarli uno a uno, alcuni sono hit-area o handle di drag e vanno lasciati
- [ ] `Card` e `Label` sono installati in `src/components/ui/` ma non usati da nessun file applicativo; `Badge`, `Alert` ed `Empty` non sono installati benché ci siano span/div stilizzati che li rimpiazzerebbero
- [x] Colonna destra (Layer Inspector) convertita da `<aside>` a `<Sidebar side="right">`: transizione di apertura invece dello smontaggio secco, `data-state`/`data-side`, `SidebarContent`, e riuso di `SidebarResizeHandle` (nuova prop `side`) al posto delle classi duplicate inline
- [x] `sidebar.tsx`: prop `keyboardShortcut` e `cookieName` (default invariati) per far convivere due `SidebarProvider` senza che ⌘B muova entrambe le colonne e senza che si sovrascrivano il cookie. Divergenza dal file generato: un `shadcn add sidebar` la sovrascriverebbe
- [ ] Nessuna delle due sidebar ricorda lo stato al reload: il cookie `sidebar_state` viene scritto ma mai letto (`defaultOpen` è sempre `true`), perché quel meccanismo è pensato per l'SSR di Next.js. Se serve la persistenza, leggere il cookie all'avvio o usare localStorage come `playlistVisible`
- [ ] Valutare se dare alla colonna destra una scorciatoia propria (es. ⌘J): oggi ha solo il pulsante in `TopToolbar`, per scelta esplicita
- [x] Titolo della colonna destra spostato in `<SidebarHeader>` (simmetrico all'header col logo a sinistra) e allineato a `h-12`: le tre fasce in cima ora chiudono tutte a 48px, prima quella destra era 49
- [x] Icona del toggle della colonna destra da `PanelRight` a `Layers`: dice cosa contiene il pannello, non dove si apre
- [x] **Risolto il bug preesistente dell'overflow della `TopToolbar`**: le etichette della nav spariscono sotto soglia e restano le sole icone (310px liberati, ne mancavano 38). Soglia sulla larghezza della toolbar via `@container`, non sulla finestra, così reagisce anche all'apertura/chiusura delle colonne a finestra ferma. Due valori perché in Live compare "Esegui in output". Il nome dei pulsanti è ora nel `title`
- [x] Layer Inspector: il divisorio sotto la lista dei layer passa da `border-t` della `ScrollArea` a `border-b` del blocco fisso. Il bordo non scorreva (era sulla Root, immobile): lo copriva la sfumatura, che parte dal suo stesso pixel ed è opaca in cima. Ora sta sopra di essa e resta sempre visibile
- [ ] Stesso difetto nella colonna **sinistra**: il titolo del pannello ha `border-b` e la `ScrollArea` ha anche un `border-t`, entrambi sul pixel y=97. A riposo sono due righe adiacenti, scorrendo la sfumatura ne copre una e il bordo cambia spessore. Il fix è identico: togliere il `border-t` alla `ScrollArea` in `ControlPage.tsx`
