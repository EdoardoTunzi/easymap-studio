...
- [ ] **Mesh warp a griglia NxM** (2×2 → 5×5 di nodi trascinabili, i "pin point a piacere"): terza fase concordata, si innesta sullo stesso campo `warp` come modalità alternativa alle Bézier
- [ ] Keystone numerico H/V + correzione barrel/pincushion (slider, per proiettori a ottica corta)
- [ ] Soft edge / feather sul perimetro del quad (bordo della proiezione sfumato invece che netto)
- [ ] Undo/redo del mapping (oggi un drag sbagliato non si annulla: c'è solo il lucchetto come prevenzione)
- [ ] Input numerico dei corner + preset di mapping salvabili separati dal progetto (stesso palco, asset diversi)
